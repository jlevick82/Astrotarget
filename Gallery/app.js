...
const els = {
  ...
  cardTpl: document.querySelector('#cardTpl')
};

function loadBaseUrl(){
  const saved = localStorage.getItem('astro.baseUrl');
  els.base.value = saved || (window.__ASTRO_CONFIG__?.defaultBaseUrl || '');
}
function saveBaseUrl(){
  localStorage.setItem('astro.baseUrl', els.base.value.trim());
...
async function testPath(){
  ...
  try{ const res = await fetch(url, { method: 'HEAD' });
       if(res.ok){ toast('Path OK'); } else { toastBad('Path not found (HEAD '+res.status+')'); } }
  catch(e){ toastBad('Network error'); }
}
function nameFor(cat, n){ return cat==='messier'?`m${n}`:(cat==='caldwell'?`c${n}`:`ngc${n}`); }
function makeUrl(cat, n, hiRes=true, attempt=0){
  const base = (els.base.value||'').replace(/\s+/g,'').trim(); if(!base) return '';
  const name = nameFor(cat,n);
...
async function loadRange(){
  els.grid.innerHTML='';
  const start = Math.max(1, parseInt(els.start.value,10)||1);
  const end = Math.max(start, parseInt(els.end.value,10)||start);
  const cat = els.catalog.value;
  const max = cat==='messier'?110:(cat==='caldwell'?109:9999);
  const s = Math.min(start, max), e = Math.min(end, max);

  for(let n=s; n<=e; n++){
    const label = nameFor(cat,n).toUpperCase();
    const node = cardElement(label);
    const img = node.querySelector('img');
    const miss = node.querySelector('.missing');
    let loaded = false;
    for(let attempt=0; attempt<3; attempt++){
      const url = makeUrl(cat, n, els.hires.checked, attempt);
      if(!url) break;
      try{
        const res = await fetch(url, { cache: 'force-cache' });
        if(res.ok && res.headers.get('content-type')?.includes('image')){
          img.src = URL.createObjectURL(await res.blob());
          img.alt = label; loaded = true; break;
        }
      }catch(e){}
    }
...
async function updateCacheInfo(){
  try{ const keys = await caches.keys(); let total=0; for(const k of keys){ const c = await caches.open(k); total += (await c.keys()).length; } els.cacheInfo.textContent = `cache: ${total} item(s)`; }
...
document.querySelector('#installBtn')?.addEventListener('click', async ()=>{ if(!deferredPrompt) return; deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if(outcome==='accepted'){ els.installBtn.hidden=true; } });
...
loadBaseUrl(); netStatus(); updateCacheInfo();
