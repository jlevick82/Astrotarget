...
/* --- Moon / Sun calculations (low-accuracy, good enough for scoring) --- */
...
function sunEq(date){
  ...
  return { raH: (ra<0?ra+2*Math.PI:ra)*12/Math.PI, decD: r2d(dec) };
}

function moonEq(date){
  const J = jd(date); const T = (J-2451545.0)/36525;
  const L = (218.316 + 13.176396* (J-2451545.0))%360;    // mean ecliptic longitude
  const M = (134.963 + 13.064993* (J-2451545.0))%360;    // mean anomaly
  const F = (93.272 + 13.229350* (J-2451545.0))%360;     // argument of latitude
  // Ecliptic longitude & latitude (very rough series)
  const lon = L + 6.289*Math.sin(d2r(M));
  const lat = 5.128*Math.sin(d2r(F));
  const eps = 23.439 - 0.0000004*(J-2451545.0);
  ...
  let raH = (ra<0?ra+2*Math.PI:ra)*12/Math.PI;
  return { raH, decD: r2d(dec), lonD: lon%360 };
}
...
async function computeMoonBrief(){
  try{
    ...
    // use current time for splash summary
    const now = new Date();
    const s = sunEq(now); const m = moonEq(now);
    const altM = altFor(m.raH, m.decD, lat, lon, now);
    // Illumination fraction via phase angle approximation
    const phase = (m.lonD - (s.raH*15))%360;
    const k = (1 - Math.cos(d2r(phase)))/2; // 0=new, 1=full
    MOON_CACHE = { now, sun:s, moon:m, altM, illum:k };
    if(moonBrief) moonBrief.textContent = `Moon illum ~ ${(k*100).toFixed(0)}% • Alt ~ ${altM.toFixed(0)}°`;
  }catch(e){
    if(moonBrief) moonBrief.textContent = 'Moon data unavailable';
    throw e;
  }
}
...
function computeTop5(){
  ...
  function moonPenalty(raH, decD){
    ...
    // Moon high makes it worse; scale by illumination
    const highBonus = altM > 45 ? 0.15 : 0.0;
    return (p + highBonus) * illum; // 0..~0.75
  }
