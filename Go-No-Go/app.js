/* Dashboard-only (no splash), optics presets, moon-aware scoring */
...
function init(){ const U=ui();
  ...
  const Llat=localStorage.getItem('gng.last.lat'), Llon=localStorage.getItem('gng.last.lon'); if(Llat && Llon){ U.lat.value=Llat; U.lon.value=Llon; }
...
function updateMoonInfo(){ const U=ui(); const lat=parseFloat(U.lat.value)||0, lon=parseFloat(U.lon.value)||0;
  ...
  ui().moonInfo.textContent=`Moon: ${(illum*100).toFixed(0)}% • Alt ${altM.toFixed(0)}° at ${mid.toLocaleTimeString()}`; return {s,m,altM,illum}; }
