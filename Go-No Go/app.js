/* Planner with vector logo splash + per-scope factor presets */
...
async function runSteps(){
  for(const st of steps){
    ...
    try{ await st.run(); row.spin.remove(); row.mark.className='mark ok'; row.mark.style.visibility='visible';
      progress += 100/steps.length; bar.style.width=Math.min(100,progress)+'%'; }
    catch(e){ row.spin.remove(); row.mark.className='mark fail'; row.mark.style.visibility='visible'; console.warn(st.key, e); }
  }
  setTimeout(()=>{ splash.classList.add('fade-out'); setTimeout(()=>{ splash.classList.add('hidden'); app.classList.remove('hidden'); }, 600); }, 250);
}
...
function checkGeo(){ return new Promise((res,rej)=>{
  if(!navigator.geolocation){ rej(new Error('no geolocation')); return; }
  navigator.geolocation.getCurrentPosition(pos=>{ const U=ui(); U.lat.value=pos.coords.latitude.toFixed(5); U.lon.value=pos.coords.longitude.toFixed(5); res(); },
    err=>rej(err), { enableHighAccuracy:true, timeout:4000 });
});}
function checkTZ(){ try{ $('#tz').textContent = Intl.DateTimeFormat().resolvedOptions().timeZone || '—'; return Promise.resolve(); } catch(e){ return Promise.reject(e); } }
async function pingWeather(){ try{ const r = await fetch('https://api.open-meteo.com/v1/forecast?latitude=0&longitude=0&hourly=cloud_cover&timezone=auto',{method:'HEAD'}); if(!r.ok) throw new Error('wx '+r.status); } catch(e){ const r = await fetch('https://api.open-meteo.com/v1/forecast?latitude=0&longitude=0&hourly=cloud_cover&timezone=auto'); if(!r.ok) throw new Error('wx get'); } }
function loadCatalog(){ if(!Array.isArray(window.MESSIER_SUBSET)) throw new Error('no catalog'); return Promise.resolve(); }

/* Astronomy helpers (approx) */
...
function sunEq(date){ const J=jd(date),T=(J-2451545)/36525; const L0=(280.46646+36000.76983*T+0.0003032*T*T)%360; const M=(357.52911+35999.05029*T-0.0001537*T*T)%360;
  ...
  const lam=L0+C, eps=23.439291-0.0130042*T; const ra=Math.atan2(Math.cos(d2r(eps))*Math.sin(d2r(lam)),Math.cos(d2r(lam))); const dec=Math.asin(Math.sin(d2r(eps))*Math.sin(d2r(lam)));
...
function moonEq(date){ const J=jd(date); const L=(218.316+13.176396*(J-2451545))%360, M=(134.963+13.064993*(J-2451545))%360, F=(93.272+13.229350*(J-2451545))%360;
  const lon=(L+6.289*Math.sin(d2r(M)))%360, lat=5.128*Math.sin(d2r(F)), eps=23.439;
...
function altFor(raH, decD, latD, lonD, date){ const LST=lst(date,lonD)*15; const HA=((LST-raH*15+540)%360)-180;
  const ha=d2r(HA), dec=d2r(decD), lat=d2r(latD); const sA=Math.sin(dec)*Math.sin(lat)+Math.cos(dec)*Math.cos(lat)*Math.cos(ha); return r2d(Math.asin(sA)); }
...
async function computeMoonBrief(){ try{ const U=ui(); const lat=parseFloat(U.lat?.value)||0, lon=parseFloat(U.lon?.value)||0; const now=new Date();
  const s=sunEq(now), m=moonEq(now); const altM=altFor(m.raH,m.decD,lat,lon,now); const phase=(m.lonD-s.lamD+360)%360; const k=(1-Math.cos(d2r(phase)))/2;
  const mb=$('#moonBrief'); if(mb) mb.textContent=`Moon ${(k*100).toFixed(0)}% • Alt ${altM.toFixed(0)}°`; }catch(e){ const mb=$('#moonBrief'); if(mb) mb.textContent='Moon data unavailable'; } }
...
function ui(){ return {
  scopeSel:$('#scopeSel'), scopeFL:$('#scopeFL'), scopeAp:$('#scopeAp'), optFactor:$('#optFactor'),
  camSel:$('#camSel'), camW:$('#camW'), camH:$('#camH'), camPx:$('#camPx'),
  chips:$('#chips'), savePreset:$('#savePreset'),
...
function initPlanner(){
  ...
  U.tz.textContent=Intl.DateTimeFormat().resolvedOptions().timeZone; loadSaved(); updateOptics(); bindPlanner(); updateMoonInfo();
}

function renderChips(){
  const U=ui(); U.chips.innerHTML='';
  const s=scopes[parseInt(U.scopeSel.value,10)];
  // Load custom per-scope preset if any
  const storeKey=`gng.preset.${s.id}`; const custom=localStorage.getItem(storeKey); const list=[...s.presets];
  if(custom){ const val=parseFloat(custom); if(Number.isFinite(val) && !list.includes(val)) list.unshift(val); }
...
function highlightChip(v){
  const U=ui();
  [...U.chips.querySelectorAll('button')].forEach(b=>{ b.classList.toggle('active', b.textContent===`${v}×`); });
}

function bindPlanner(){
  const U=ui();
  U.scopeSel.addEventListener('change', ()=>{applyScope(); renderChips(); updateOptics();});
  ...
  U.savePreset.addEventListener('click', ()=>{
    const s=scopes[parseInt(U.scopeSel.value,10)];
    const factor=parseFloat(U.optFactor.value)||1;
    localStorage.setItem(`gng.preset.${s.id}`, String(factor)); renderChips();
  });
  renderTable();
}
...
function updateOptics(){
  const U=ui();
  const fl=parseFloat(U.scopeFL.value)||0; const factor=parseFloat(U.optFactor.value)||1; const eff=fl*factor;
  const sw=parseFloat(U.camW.value)||0; const sh=parseFloat(U.camH.value)||0; const px=parseFloat(U.camPx.value)||0;
  U.effFL.textContent = eff>0 ? `Eff. FL: ${eff.toFixed(0)} mm` : 'Eff. FL: —';
  if(eff>0 && sw>0 && sh>0){ const fovW=57.2958*sw/eff, fovH=57.2958*sh/eff; U.fov.textContent=`FOV: ${fovW.toFixed(1)}° × ${fovH.toFixed(1)}°`; } else { U.fov.textContent='FOV: —'; }
  if(eff>0 && px>0){ const scale=206.265*px/eff; U.scale.textContent=`Scale: ${scale.toFixed(2)}″/px`; } else { U.scale.textContent='Scale: —'; }
}

function saveSetup(){
  const U=ui();
  const data={ scope:{fl:U.scopeFL.value, ap:U.scopeAp.value, sel:U.scopeSel.value, factor:U.optFactor.value},
...
/* Weather + Recs (same as v2) */
async function useGPS(){ const U=ui(); try{ U.gps.disabled=true;
  const pos=await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:10000}));
  U.lat.value=pos.coords.latitude.toFixed(5); U.lon.value=pos.coords.longitude.toFixed(5);
} catch(e){ alert('GPS failed: '+e.message); } finally{ U.gps.disabled=false; } }

async function fetchWeather(){ const U=ui(); const lat=parseFloat(U.lat.value), lon=parseFloat(U.lon.value);
  if(!Number.isFinite(lat)||!Number.isFinite(lon)){ U.wxStatus.textContent='Set lat/lon first'; return; }
  U.wxStatus.textContent='Loading…';
  try{ const r=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=cloud_cover,visibility,temperature_2m,relative_humidity_2m&timezone=auto`);
    const j=await r.json(); U.wxStatus.textContent='Updated'; renderWx(j); computeTop5();
...
function renderWx(j){ const U=ui(); if(!j || !j.hourly){ U.wx.innerHTML='<span class="small">No weather.</span>'; return; }
  ...
  for(const r of t){ html+=`<tr><td>${r.time}</td><td>${r.cloud}</td><td>${r.vis.toFixed(0)}</td><td>${r.temp}</td></tr>`; } html+='</tbody></table>'; U.wx.innerHTML=html; }
...
function gmst(date){ const J=jd(date),T=(J-2451545)/36525; let th=280.46061837+360.98564736629*(J-2451545)+0.000387933*T*T-T*T*T/38710000; th=((th%360)+360)%360; return th/15; }
function lst(date, lon){ return (gmst(date)+lon/15+24)%24; }
function sunEq(date){ const J=jd(date),T=(J-2451545)/36525; const L0=(280.46646+36000.76983*T+0.0003032*T*T)%360;
  const M=(357.52911+35999.05029*T-0.0001537*T*T)%360; const C=(1.914602-0.004817*T-0.000014*T*T)*Math.sin(d2r(M))+(0.019993-0.000101*T)*Math.sin(d2r(2*M))+0.000289*Math.sin(d2r(3*M));
  const lam=L0+C, eps=23.439291-0.0130042*T; const ra=Math.atan2(Math.cos(d2r(eps))*Math.sin(d2r(lam)),Math.cos(d2r(lam))); const dec=Math.asin(Math.sin(d2r(eps))*Math.sin(d2r(lam)));
  return {raH:(ra<0?ra+2*Math.PI:ra)*12/Math.PI, decD:r2d(dec), lamD:lam}; }
function moonEq(date){ const J=jd(date); const L=(218.316+13.176396*(J-2451545))%360, M=(134.963+13.064993*(J-2451545))%360, F=(93.272+13.229350*(J-2451545))%360;
  const lon=(L+6.289*Math.sin(d2r(M)))%360, lat=5.128*Math.sin(d2r(F)), eps=23.439;
...
function updateMoonInfo(){ const U=ui(); const lat=parseFloat(U.lat.value)||0, lon=parseFloat(U.lon.value)||0;
  ...
  U.moonInfo.textContent=`Moon: ${(illum*100).toFixed(0)}% • Alt ${altM.toFixed(0)}° at ${mid.toLocaleTimeString()}`; return {s,m,altM,illum}; }
...
function computeTop5(){
  ...
  const s=sunEq(mid); const altSun=altFor(s.raH,s.decD,lat,lon,mid); if(altSun>-12){ U.top5.innerHTML='<li class="bad">Astronomical darkness not reached.</li>'; return; }
  ...
  U.top5.innerHTML = scored.length ? scored.map(o=>`<li><strong>${o.id}</strong> — ${o.name} • alt ${o.alt.toFixed(0)}° • sep ${o.sep?.toFixed(0)}° • ${(o.score*100).toFixed(0)}%</li>`).join('') : '<li class="bad">No suitable targets.</li>';
  updateMoonInfo(); renderTable();
}
...
/* Splash controls */
$('#skip').addEventListener('click', ()=>{ splash.classList.add('fade-out'); setTimeout(()=>{ splash.classList.add('hidden'); app.classList.remove('hidden'); }, 600); });
